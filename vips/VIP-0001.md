...
hop on the hot path). Each pod sizes its own bucket to a **static even budget** of the topic's
...
flowchart TD
    ...
    RB -->|"÷ podCount × (1+buffer), floored<br/>(computed locally on each pod)"| PS["Per-pod budget"]
...
**adaptive coordinator** that reshapes budgets to follow skewed traffic is fully designed but
...
| Server pods / region | 10 – 50 (dynamic) | `podCount` from membership; budget recomputed on membership change. |
| Per-topic QPS | Most < 1k; a few 10k–100k | Even split works for hot topics; low-QPS topics need the `minPodQps` floor (§9). |
...
| **B — Local enforcement + adaptive budgets** | Pods enforce locally; a per-region controller recomputes each pod's budget from observed demand and pushes it back out-of-band. | Adapts to skew, but adds a coordinator, a new bidirectional protocol, AIMD control, and oscillation risk. Its main justification (skew) is absent in the v1 deployment. **Deferred to a data-gated future phase (§18).** |
...
in-memory enforcement, no hot-path network hop, correct for hot and (with the `minPodQps`
...
flowchart LR
    ...
    subgraph Region["Regional cluster"]
        ...
        subgraph Servers["varadhi-server fleet (10-50 pods)"]
            P1["pod · produce-rate-limiter<br/>perPodQuota = regionBudget ÷ podCount<br/>(computed locally)"]
...
- **No controller-side RL component in v1.** Each pod sizes its own budget locally; the only
...
identical whether the budget comes from the v1 static split or a future coordinator.
...
- **Burst control.** Bucket capacity = `rate × windowSecs` (`windowSecs` default ~1s).
  Continuous refill means no large discrete window; worst-case micro-burst is bounded by
  `windowSecs`. Capacity stays **coupled to rate**.
- **Budget updates** are a cheap volatile write of the refill rate — no hot-path allocation.
...
- **Both `qps` and `throughputKBps` are floored at 1** — at topic-creation capacity planning
  *and* when a pod computes its per-pod budget (a pod is never given less than the minimum).
- **A qps floor implies a bytes floor.** For a `1 qps` floor to be *honest* (the one message
  ...
  deliverable qps budget `q` requires bytes budget `≥ q × msgSize`.
- **Two metrics, two uses:**
  ...
  - **max (or p99) msg size** → the *bucket-capacity floor* (`bytesQuota ≥ qpsQuota × maxMsgSize`)
...
- **Both buckets are still enforced at runtime.** The coupling is a *budget-sizing* input, not a
...
## 9. Per-Pod Budget — Static Even Split (the v1 mechanism)

Every pod computes its own budget for every topic, locally, with no coordination:

```
regionBudget(topic) = globalQuota(topic) × regionWeight(topic, region)        # static config
perPodQuota(topic)  = regionBudget ÷ podCount × (1 + fallbackBuffer)          # divide by membership
perPodQuota(topic)  = max(perPodQuota, minPodQps)                           # floor (low-QPS)
bytesQuota(topic)   = max(bytesQuota, qpsQuota × maxMsgSize)                  # dimension coupling
```
...
- **Why an even split is sufficient here:** under approximately-even LB distribution (§4.1),
  each pod receives ≈ `1/podCount` of the topic's traffic, so an even budget ≈ each pod's actual
  load. Hot topics split cleanly; the only approximation cost is on low-QPS topics (below).
- **Low-QPS topics and the `minPodQps` floor.** Splitting a small quota across many pods makes
  ...
  `minPodQps` floor (e.g. 1 qps) prevents this. **Cost:** the aggregate allowance becomes
  `podCount × minPodQps`, i.e. bounded over-allow for small topics — operationally harmless
  (small absolute numbers; the rationale that "small topics barely affect cluster health") and
  the explicit, bounded price of being coordinator-free. Keep `minPodQps` small.
- **Membership transients.** When a pod joins/leaves, every pod recomputes `podCount` from ZK
  and resizes; brief over-/under-allow during convergence errs safe for a guard-rail. `budgetTtl`
...
- **Per-region weight** does **not** go in `TopicCapacityPolicy` (that class is a flat,
  ...
  region-keyed `Map<String, Double> perRegionQuotaWeights` on `VaradhiTopic`, parallel to its
...
- **Minimum capacity capping.** `qps` and `throughputKBps` each floored at **1**, enforced at
  topic-creation *and* in every per-pod budget computation. Effective bytes floor is
...
| `windowSecs` | ~`1` | Token-bucket capacity = `rate × windowSecs`. |
| `minPodQps` | small (e.g. 1) | Per-pod floor so low-QPS topics aren't starved (§9). Bounds over-allow at `podCount × minPodQps`. |

> The coordinator-specific knobs from earlier drafts (`reportInterval`, `resyncInterval`,
> `budgetTtl`, `missedReportGraceIntervals`, `coordinationThreshold`, `demandQuantizationPct`,
...
| Pod just started | Compute the budget immediately from cached config + current membership; no warm-up dependency. |
...
- **`Σ regionWeight` invariant + validation.** Since `regionBudget = globalQuota × regionWeight`,
  ...
  `perRegionQuotaWeights` (how unset regions default; how the even split recomputes when a produce
...
- **Regional failover strands quota (known v1 limitation).** Static `regionWeight` means a
  ...
  **operator-adjustable via UI**, intended to be driven by the **zone-unavailability feature**
  ...
  weights are §18.)
- **Bounded over-allow is intentional.** `podCount × minPodQps` for small topics, plus brief
...
- **Per-pod, per-topic** (via existing produce telemetry → otel-collector): accepted qps,
  accepted bytes-rate, rejected qps, rejected bytes-rate, current computed budget (qps + bytes),
...
- Token-bucket `tryAcquire`: refill via **monotonic clock**; **credit-based admission** (admit
  on `both > 0`, debit both, never cost-coverage); burst bound; budget update.
- **Dimension coupling:** bytes budget floored at `qpsQuota × maxMsgSize` ⇒ a single max-size
  message fits without debt; deep bounded-debt only as a residual, bounded by `maxRequestSize`.
- **Static split:** `regionBudget ÷ podCount × (1+buffer)` floored at `minPodQps`; recompute on
  membership change; worst-case over-allow `= podCount × minPodQps`.
- **`perRegionQuotaWeights` validation:** sum-to-1, partial-map defaults.
...
1. **Phase 1 — local enforcement with static split (the v1 product).** Daemonless token-bucket
   limiter in `produce-rate-limiter`, budget computed locally as `regionBudget ÷ podCount`, behind
...
2. **Phase 2 — adaptive coordination, only if data demands it (§18).** Built **only if** the
   ...
   only the *source* of each pod's budget changes from local-compute to controller-assigned.
...
> configuration/observability above stay the same; **only the source of each pod's budget changes
...
demand and recomputes per-pod budgets periodically, pushing them back **out-of-band** (never per
...
sequenceDiagram
    ...
    loop every interval
        Controller-->>Pod: budget deltas {topicId → qps/bytes budget}
    end
...
Adaptive budgets concentrate quota on the pods actually receiving a topic's traffic. This only
...
- **Pod → controller report** (~5s, only topics with traffic): `accepted` qps+bytes **+ a
  ...
  interval's observed `avg`/`max` size (for the bytes-budget floor). Keyed by interned int
  topic-ID; optionally quantized by demand change.
- **Controller computation** (AIMD per topic): each pod's budget floors at its reported
  `accepted`; a `saturated` pod's budget grows (additive), unused budget is reclaimed from
  non-saturated pods (multiplicative), bounded by `regionBudget`. When *all* pods are saturated
  (topic legitimately over quota) there is nothing to reclaim — budgets settle at the budget split
  and excess is correctly 429'd. Every active pod keeps a dynamic `minPodQps` floor; bytes
  budget floored at `qpsQuota × maxMsgSize`.
- **Controller → pod** (~5s): `topicId → {qps, bytes}` as **deltas** (only changed budgets) + a
  periodic **full resync** (~60s / on reconnect). Per-interval cost tracks **churn, not topic
  count**.
- **Lifecycle:** reclaim authority is ZK membership (a departed pod's budget is reclaimed next
  cycle); a pod alive-but-silent is reclaimed only after `missedReportGraceIntervals` (K), and a
  pod **always keeps enforcing its last-known budget locally**, so a blind controller can never
  drive a busy pod to zero. On controller-down, pods hold last-known budgets until `budgetTtl`
  (sized ≥ measured worst-case controller failover; a stale budget is safer than the floor), then
...
metrics** first (budget churn / interval; 429-on-under-budget). Build **EWMA** smoothing
(controller-side) but ship it **off**, flipped per topic via `oscillationProtection`, as an A/B
lever. Escalate to deadband/hysteresis (`budgetDeadbandPct`) and capped adjustment
(`budgetAdjustAlpha`, `maxBudgetIncreasePerCycle`) only if EWMA proves insufficient.
...
- **Bidirectional** messaging (reports up, distinct per-pod budgets down).
...
- **Dictionary epoch guard (mandatory correctness rule):** every frame carries the handshake
  **epoch**; a pod **drops any frame whose epoch ≠ its current epoch**, so a frame built under an
  old ID-space (after a controller rebuild) can never land budgets on the wrong topics.
...
- **`minPodQps` default** and the accepted `podCount × minPodQps` over-allow bound.
- **`fallbackBuffer` / `windowSecs` defaults** from hot-path and jitter measurements.
...
- **`perRegionQuotaWeights` validation + partial-map semantics** (sum-to-1; default-even recompute
...
- The empirical skew threshold (from the §15 metric) at which adaptive coordination is worth
  building.
- If/when built: coordinator tunables (`reportInterval`, `resyncInterval`, `budgetTtl`,
  `missedReportGraceIntervals`, `coordinationThreshold`, `demandQuantizationPct`, dynamic
  `minPodQps`), the binary codec + exact wire framing, and whether `oscillationProtection`
  (EWMA) is needed.
