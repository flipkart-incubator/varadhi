...
`PENDING → PREPARE → SWITCH → DRAIN → COMPLETED` (or `ABORTED`) driven by the controller.
Pods receive self-contained `TransitionEvent` broadcasts, pre-warm at PREPARE, fence at SWITCH,
and ack via `TransitionAck`. The **only routing input on pods** is `VaradhiTopic` in
...
- Work for **grouped and ungrouped** topics; replication-lag wait stricter for grouped.
...
- **Legacy `internalTopics` JSON migration** — tracked as follow-up; deserializer backfill TBD.
...
|---|---|---|---|---|
| T1 | **Planned DR** | Operator REST `POST .../failover` | Orchestrated PREPARE→SWITCH→DRAIN | ✅ v1 |
...
| `DRAIN` | No (controller-only) | Steady on new authority | Poll replication lag |
...
| Grouped | `true` | Mostly ordered (`X_GROUP_ID`); **replication-lag wait required** before COMPLETED |
...
| E8 | Replication lag > 0 at SWITCH | Optional preflight block; DRAIN waits until zero |
| E9 | `skipValidation=true` on REST | Operator override for emergency (documented risk) |
| E10 | Idempotent flip to same state | 200 no-op |
| E11 | Auto-failover cooldown | Per-topic / per-region cooldown prevents flap |
| E12 | Global freeze | `/varadhi/failover/freeze=true` kill-switch blocks auto (manual may override) |
| E13 | `autoFailover=false` on topic | Skip in automatic region batch |
| E14 | Legacy JSON without `produceConfigs` | Deserializer backfill TBD |
...
| Area | Requirement |
|---|---|
| **Availability** | Produce outage bounded to SWITCH fence window, not full PREPARE+DRAIN |
...
| **Migration** | New wire shape `produceConfigs`; legacy `internalTopics` backfill TBD |
...
| `TransitionStage` | `PENDING`, `PREPARE`, `SWITCH`, `DRAIN`, `COMPLETED`, `ABORTED` |
...
|---|---|---|
| `POST` | `/v1/projects/:p/topics/:t/failover` | Body: `{ toRegion, waitForReplicationLagToClear?, skipValidation? }` → `200` + `opId` |
| `GET` | `/v1/projects/:p/topics/:t/failover` | Active transition snapshot (404 if none) |
| `POST` | `/v1/projects/:p/topics/:t/failover/abort` | Only while `PENDING` or `PREPARE` |
...
| Global freeze flag | Emergency stop |
...
| Gate | Level | Effect |
|---|---|---|
| `autoFailover` on topic | Topic | Allows automatic region-driven failover |
| Manual REST | Always (with auth) | Operator can failover regardless of `autoFailover` |
| `skipValidation` | Per request | Bypass replication-lag preflight (emergency) |
| Global freeze | Deployment | Blocks automatic only |
...
| Replication lag never clears | DRAIN timeout → ABORTED or operator override |
...
| Legacy topic JSON | Backfill TBD; fail closed if `produceConfigs` empty |
...
- Stage guard logic: abort only in `PENDING`/`PREPARE`; all-dark detection.

### Integration

- Manual failover happy path: PREPARE warm → SWITCH fence → DRAIN → COMPLETED.
- Abort before SWITCH: no Topic version change.
...
- Grouped topic: replication-lag gate enforced.
...
| W10 | Legacy JSON migration | entities | — |
...
7. Legacy `internalTopics` migration: inline deserializer vs offline job?

### Cutover

8. Local-only vs cross-region produce as default?
9. Is replication-lag SPI required for v1 or optional?
10. PREPARE pre-warm: separate cache namespace vs in-place producer swap?
11. Exact HTTP status for concurrent failover (409 proposed)?

### Automation (deferred)

12. Cooldown defaults (per topic, per region)?
13. `HIGH_PRIORITY` tag ordering in region batch?
14. Empirical gate for enabling auto-failover in production?

### Ops

15. Multi-pod ack barrier validation beyond single-pod harness?
16. Consumer IQ/RQ/DLQ pods in same barrier as Server — confirmed?
17. Failback default on region recovery?
