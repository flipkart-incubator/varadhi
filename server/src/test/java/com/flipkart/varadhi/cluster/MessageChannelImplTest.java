...
public class MessageChannelImplTest {
    ...
    public void setup(){
        ...
        JsonMapper.getMapper().registerSubtypes(new NamedType(ExtendedTestClusterMessage.class, "ExtendedTestClusterMessage"));
    }
    ...
    private Vertx createClusteredVertx() throws Exception {
        ...
        vertxOptions.setClusterManager(cm);
        return Vertx.clusteredVertx(vertxOptions).toCompletionStage().toCompletableFuture().get();
    }
    @Test
    public void sendMessageNoConsumer(VertxTestContext testContext) throws Exception {
        Checkpoint checkpoint =  testContext.checkpoint(1);
        Vertx vertx = createClusteredVertx();
        MessageChannelImpl c = new MessageChannelImpl(vertx);
        ClusterMessage cm = getClusterMessage("foo");
        Future.fromCompletionStage(c.send("foo", cm)).onComplete(testContext.failing(v -> {
            checkpoint.flag();
        }));
    }

    @Test
    public void testSendMessageConsumerColocated(VertxTestContext testContext) throws Exception{
        ...
        ClusterMessage cm = getClusterMessage("foo");
        Future.fromCompletionStage(c.send("foo", cm)).onComplete(testContext.succeeding(v -> {
            checkpoint.flag();
        }));
    }
